...
enum ProductLimits {
    ...
    static let body = """
EscapeSpace 通过 LocalDevVPN 和配对文件列出已安装的应用（支持 iOS 18 与 iOS 26），然后打开其他应用的数据容器：Documents、Library 和 tmp。House Arrest 只是让电脑能把那份配对文件放进 EscapeSpace 的 Documents，并不能替代配对，也不能从 EscapeSpace 内部列出或打开其他应用。
