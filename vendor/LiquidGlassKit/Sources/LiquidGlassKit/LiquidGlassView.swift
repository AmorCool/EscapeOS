...
final class LiquidGlassRenderer {
    ...
    /// EscapeSpace 适配（Theos 构建）：shader 库不可用时不崩溃，
    /// `isAvailable` 为 false，视图退化为普通渲染（不显示玻璃效果）。
    var isAvailable: Bool { device != nil && pipelineState != nil }

    private init() {
        // 1. Metal 设备（不支持时降级，不 fatalError）
        ...
        // 2. shader 库：Theos 把编译好的 metallib 打进 App 主 bundle
        //    （原版依赖 SwiftPM 的 .module 或资源 bundle，Theos 下都不存在，
        //    原实现用 try! 会直接崩溃）。
        var library: MTLLibrary?
        if let url = Bundle.main.url(forResource: "LiquidGlassKit", withExtension: "metallib") {
...
//        draw(bounds)
    ...
    override func draw(_ rect: CGRect) {
        ...
        guard let pipelineState = LiquidGlassRenderer.shared.pipelineState else {
            encoder.endEncoding()
            return
        }
